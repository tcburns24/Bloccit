...
RSpec.describe CommentsController, type: :controller do
  ...
  context "member user doing CRUD on a comment they own" do
    ...
    describe "POST create" do
      it "increases the number of comments by 1" do
        expect{ post :create, format: :js, post_id: my_post.id, comment: {body: RandomData.random_sentence} }.to change(Comment,:comment).by(1)
      end

      it "returns http success" do
        post :create, format: :js, post_id: my_post.id, coomment: {body: RandomData.random_paragraph}
        expect(response).to have_http_status(:success)
      end
    end
